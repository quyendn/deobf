...
(async () => {
    try {
        ...
        const resourceLinkMatch = link.match(/https:\/\/([^/]+)\/embed-1\/v2\/e-1\/([^?]+)/);
        ...
        const obfuscatedJS = await fetchUrl(`/js/player/m/v2/pro/embed-1.min.js?v=${Math.floor(Date.now() / 1000)}`);
        ...
        const embedContentRaw = await fetchUrl(`/embed-1/v2/e-1/getSources?id=${ID}`, { 'referer': `${baseUrl}/embed-1/v2/e-1/getSources?id=${ID}`});
